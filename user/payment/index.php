...
<html lang="en">
  ...
  <head>
    ...
    <!-- Site Title  -->
    <title>Payment - Apexholdco</title>
...
<div class="px-lg-5 mt-4">
    <div class="post-box p-0 pb-lg-4 bg-sm-none">
      <div class="d-flex border-bottom p-3 border-mobile-none">
        <p class="m-0 roboto xsmall-sm bold-text flex-fill">Deposit <?=  $transactions['amount'] . ' '. $transactions['currency'];?></p>
        <p class="m-0 ml-3 primary-color xsmall">Complete payment</p>
      </div>
      <div class="row mt-4 px-md-4">
        <div class="col-lg-6 mb-3 mb-lg-0 border-bottom border-lg-none pb-4">
          <div class="post-box dark-bg bg-lg-none">
            <p class="m-0 mb-2 bold-text grey-color-2">Order Summary</p>
            <div class="d-flex">
              <p class="m-0 xsmall grey-color flex-fill">Amount</p>
              <p class="m-0 xsmall roboto"><?= $transactions['currency_symbol'] . $transactions['amount'] ;?></p>
            </div>
            <div class="d-flex mt-2">
              <p class="m-0 xsmall grey-color flex-fill">Ref</p>
              <p class="m-0 xsmall"><?= '#' . $transactions['transaction_id'];?></p>
            </div>
            <div class="d-flex mt-2">
              <p class="m-0 xsmall grey-color flex-fill">Status</p>
              <?php 
              if($transactions['status'] === 'pending'){
                  echo '<span class="badge badge-warning" title="Make payment and upload payment proof">Waiting</span>';
              }elseif($transactions['status'] === 'completed'){
                  echo '<span class="badge badge-success" title="Make payment and upload payment proof">Successful</span>';
              }
              ?>
                                
                              <!--<p class="m-0 xsmall2 primary-color add-radius px-2"-->
              <!--  style="background: #3c341f; border-radius: 4px;">-->
              <!--  Pending</p>-->
          ...
          <!-- desktop only -->
          <div class="dark-bg p-3 d-none d-md-block">
            <p class="fit-content m-0 grey-color-2 bold-text"><?= $transactions['currency'];?> (<?=  $coin['name'];?>) Deposit Address</p>
            <div class="row">
              <div class="col-9">
                <p class="xsmall m-0 bold-text" style="overflow-wrap: break-word;"><?=  $coin['wallet_address'];?></p>
              </div>
              <div class="col-3 text-center">
                <i class="material-icons grey-color-2 pointer" style="font-size: 22px !important;"
                  onclick="copyTargetAddress(this, '<?=  $coin['wallet_address'];?>')">file_copy</i>
              </div>
              <div class="col-12">
                <small class="error-text xsmall2">
                  *Send only <b><?= $transactions['currency'];?></b> to this address to avoid loss of fund.
                </small>
              </div>
            </div>
            ...
            <div class="row mt-5">
              <div class="col-6">
                  <a href="https://www.apexholdco.com/user/upload-proof/<?= $transactions['transaction_id'];?>">
                      <button class="xsmall bold-text">
                        Confirm Payment
                      </button>
                  </a>
              </div>
        ...
        <div class="col-lg-6">
          <div class="dark-bg3 p-4 add-radius">
            <div class="white-bg p-0 mx-auto" style="border-radius: 6px; width: fit-content;">
              <img
                src="https://api.qrserver.com/v1/create-qr-code/?;size=200x200&amp;margin=0&data=<?= $coin['wallet_address'];?>"
                style="padding: 20px; width: 200px;" alt="<?= $transactions['transaction_id'];?> address for transaction #<?= $coin['wallet_address'];?>" >
            </div>
            <div class="mt-4 text-center">
              <small class="xsmall2 grey-color-2">
                Please use your wallet or exchange account to send fund to the address by copying it or scanning
                the QR code; then proceed to Confirm payment.
              </small> <br>
        ...
        <!-- Mobile only -->
        <div class="col-12 dark-bg py-3 pb-5 d-block d-md-none" style="border-radius: 8px 8px 0px 0px;">
          <p class="fit-content m-0 grey-color-2"><?= $coin['symbol'];?> (<?= $coin['name'];?>) Deposit Address</p>
          <div class="row">
            <div class="col-9">
              <p class="xsmall m-0 bold-text roboto" style="overflow-wrap: break-word;">
                <?= $coin['wallet_address'];?></p>
            </div>
            <div class="col-3 text-center">
              <i class="material-icons grey-color-2" style="font-size: 22px !important;"
                onclick="copyTargetAddress(this, '<?= $coin['wallet_address'];?>')">file_copy</i>
            </div>
            <div class="col-12">
              <small class="error-text xsmall2">
                *Send only <b><?= $coin['symbol'];?></b> to this address to avoid loss of fund.
              </small>
            </div>
          </div>
 ...
 <script>
    function copyTargetAddress(e, address){
        copyToClipboard(address);
        $(e).text('task_alt');
        setTimeout(()=>{$(e).text('file_copy')}, 1200)
    }
</script>
